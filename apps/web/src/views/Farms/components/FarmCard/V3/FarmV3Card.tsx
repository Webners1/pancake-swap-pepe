...
export const FarmV3Card: React.FC<React.PropsWithChildren<FarmCardProps>> = ({
  ...
  const earnLabel = t('PEPE + Fees')
  const { lpAddress } = farm
  const isPromotedFarm = farm.token.symbol === 'PEPE'
  ...
  const aprTooltip = useTooltip(
    <>
      <Text>
        {t(
          'Global APR calculated using the total amount of active & staked liquidity with the pool PEPE reward emissions.',
        )}
      </Text>
      <br />
