...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"PEPE" | "USD">("PEPE");
  const conversionUnit = editingUnit === "PEPE" ? "USD" : "PEPE";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "PEPE" ? "USD" : "PEPE";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "PEPE") {
      ...
      setValues({
        PEPE: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
