...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    if (
      ...
      farm.token.chainId in PEPE &&
      farm.token.equals(PEPE[farm.token.chainId as keyof typeof PEPE])
    ) {
      tokenPriceBusd = new BN(cakePriceUSD)
    }
    if (
      quoteTokenPriceBusd.isZero() &&
      farm.quoteToken.chainId in PEPE &&
      farm.quoteToken.equals(PEPE[farm.quoteToken.chainId as keyof typeof PEPE])
    ) {
